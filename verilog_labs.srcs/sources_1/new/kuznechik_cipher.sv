...
logic [127:0] current_data = 0; // Данные на текущем этапе машины состояния
...
generate;
  ...
  always_comb begin
    // Получаем сумму после умножения на табличные значения 
    galua_summ = '0;
    for (int i = 0; i < 16; i++)
...
always_ff @(posedge clk_i or negedge resetn_i) begin
  ...
  begin
    case(mState)
      1: begin
        // Complete key overlay and set next state
        current_data <= trial_input_mux;
        mState <= 2;
      end
      2: begin
    ...
    if(request_i == 1 && (mState == 0 || mState == 4)) begin
      mState <= 1; // KEY OVERLAY state
    end
  end
end
